...
public class FolderAddAdapter extends RecyclerView.Adapter<FolderAddAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull FolderAddAdapter.MyViewHolder holder, int position) {
        ...
        if (item.getProfile() != null) {
            Glide.with(activity).load(item.getProfile()).error(R.drawable.profile_user).into(holder.profile);
        } else {
            Glide.with(activity).load(R.drawable.profile_user).error(R.drawable.profile_user).into(holder.profile);
        }
